...
```{r lin_reg_model, message=FALSE, warning=FALSE, collapse=TRUE}
# Model Spec
lr_spec <- linear_reg() %>%
    set_mode('regression') %>%
    set_engine('lm')
...
penalty_grid <- grid_regular(penalty(range = c(-10, 18)),
                             levels = 50)
...
final_ridge %>%
    last_fit(ames_split) %>%
    extract_fit_engine() %>%
    vip::vip(num_features = 20L, mapping = aes(fill = Sign))
```
...
# plot coefficients regularization
plot(lasso_fit$fit, xvar = 'lambda')
```
