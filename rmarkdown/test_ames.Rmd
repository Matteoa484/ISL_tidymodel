...
p1 <- nom_bar_plot(ames_train, 'Garage_Type')
p2 <- nom_bar_plot(ames_train, 'Garage_Finish')
p3 <- nom_bar_plot(ames_train, 'Garage_Qual')
p4 <- nom_bar_plot(ames_train, 'Garage_Cond')
...
p1 <- nom_bar_plot(ames_train, 'Bsmt_Qual')
p2 <- nom_bar_plot(ames_train, 'Bsmt_Cond')
p3 <- nom_bar_plot(ames_train, 'Bsmt_Exposure')
p4 <- nom_bar_plot(ames_train, 'BsmtFin_Type_1')
p5 <- nom_bar_plot(ames_train, 'BsmtFin_Type_2')
...
```{r lin_reg_model, message=FALSE, fig.align='center', out.width='90%'}
# set the model
lr_spec <- linear_reg() %>%
    set_mode('regression') %>%
    set_engine('lm')
...
# factor lump + near-zero variance
fct_rec <-
    recipe(Sale_Price ~ ., data = ames_train) %>%
...
## create model fitting workflows
...
# lr_models <- workflow_set(preproc = preproc,
...
# 
# lr_multi_test <- lr_models %>%
#     workflow_map(seed = 1101, verbose = TRUE,
#                  resamples = ames_folds, control = keep_pred)
# 
# lr_models %>% fit(data = ames_train)
# 
# 
# collect_metrics(lr_multi_test) %>%
#     filter(.metric == 'rsq')


  
# prediction plot on raw model
basic_pred_plot <- augment(basic_lr, new_data = ames_train) %>%
...
# prediction plot on pre-processed model
preproc_pred_plot <- augment(preproc_lr, new_data = ames_train) %>%
    select(Sale_Price, .pred) %>%
    ggplot(aes(x = Sale_Price, y = .pred)) +
    geom_point(alpha = .4) +
    geom_abline(lty = 2, color = 'gray50', size = 1.2) +
...
wrap_plots(basic_pred_plot, preproc_pred_plot)
```

```{r}
rmse(basic_lr$fit, truth = Sale_Price, estimate = .pred)
```


```{r}
# the log-transformation on preproc is a problem
# for the statistics
bind_rows(
    glance(basic_lr),
    glance(preproc_lr)
