...
# fit a raw linear regression to the train test set
raw_lr_fit <- lr_spec %>%
    fit(Sale_Price ~ ., data = ames_train)

# set the pre-processing recipe
lr_rec <-
...
preproc_lr_fit <- fit(lr_wf, data = ames_train)

# prediction plot on raw model
raw_pred_plot <- augment(raw_lr_fit, new_data = ames_train) %>%
...
preproc_pred_plot <- augment(preproc_lr_fit, new_data = ames_train) %>%
    select(Sale_Price, .pred) %>%
    mutate(.pred = 10^.pred) %>%
    ggplot(aes(x = Sale_Price, y = .pred)) +
    geom_point(alpha = .4) +
...
wrap_plots(raw_pred_plot, preproc_pred_plot)
```
