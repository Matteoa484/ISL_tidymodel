...
```{r}
ggplot(Boston) +
    geom_point(aes(x = lstat, y = medv), alpha = .75)
```
...
The same results can be obtained with the `augment` function from `tune`.
...
Including an interaction term can be done directly inside the formula notation using the syntax `lstat:black` to include an interaction term between `lastat` and `black` variables. The syntax `lstat*age` is a shorthand for `lstat + age + lstat:age`.
...
IN tidymodels this transformations are part of the pre-processing and thus are added to a *recipe*. The `recipe` package has the `step_mutate` function which is pretty much the equivalent of `mutate` in `dplyr`.
...
rec_spec_pow2 <- recipe(medv ~ lstat, data = Boston) %>%
    step_mutate(lstat2 = lstat ^ 2) # ploy 2 to lstat variable
...
Not all the transformations need to be done with `step_mutate` since `recipe` has a bunch of them creted already. For example `step_log` take the logarithm of the variables.
...
The following example is based on the `Carseta` data set from the package `ISLR`. We will predict `Sales` of child car seats in 400 locations based on a number of predictors. 
...
GGally::ggcorr(cor_Smarket, label = TRUE)
```
...
We cannot fit directly the model. We want to model the `Direction` of the stock market based on the percentage return from the previous 5 days plus the volume of the shares traded. `parsnip` requires that the response variable is factor, which is the case for this data set, so we don't need to do any change.
...
The model's main infor could be accessed with `summary`:
